...
class ReviewController extends Controller
{
    ...
    public function getReviewForm(Request $request){
        ...
        $data['review'] = Review::where('user_id', $user_id)->where('order_id', $request->order_id)->where('course_id', $request->course_id)->first();
    ...
    public function reviewInsert(Request $request)
    {
        ...
        $user_id = Auth::id();
        $check_user = OrderDetail::where('course_id', $request->course_id)->where('order_id', $request->order_id)->where('user_id', $user_id)->first();
        if($check_user) {
            $review = Review::where('user_id', $user_id)->where('order_id', $request->order_id)->where('course_id', $request->course_id)->first();
            if(!$review){
                $review = new Review();
            }
            $review->user_id = $user_id;
            $review->vendor_id = $check_user->vendor_id;
    ...
    public function reviewReplyList($review_id){
        $review = Review::with(['review_comments.user:id,name,username,phato', 'review_image_video'])->leftJoin('users', 'reviews.user_id', 'users.id')
            ->leftJoin('courses', 'reviews.course_id', 'courses.id')
            ->selectRaw('reviews.*, users.name as customer_name, users.username, phato, courses.title as product_name,courses.feature_image,courses.slug')
    ...
    public function reviewList()
    {
        $reviews= Review::leftJoin('users', 'reviews.user_id', 'users.id')
            ->leftJoin('courses', 'reviews.course_id', 'courses.id')
            ->selectRaw('reviews.*, users.name as student_name, users.username, courses.title as course_name,courses.thumbnail_image,courses.slug')->get();
